...
# Fallback polling interval if long polling not available
...
class ProvisionMotionSensor(BinarySensorEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if "long_polling" in self.hass.data[DOMAIN][self._entry.entry_id]:
            ...
            _LOGGER.info("Long polling not available, using fallback polling for %s", self._attr_unique_id)
            self._poll_task = asyncio.create_task(self._poll_alarm_status())
    ...
    async def _poll_alarm_status(self) -> None:
        """Poll alarm status periodically (fallback if long polling unavailable)."""
        while True:
            try:
                await asyncio.sleep(POLL_INTERVAL)
                
                # Get alarm status
                alarm_status = await self._client.get_alarm_status()
                ...
                _LOGGER.error("Error polling alarm status: %s", err)
